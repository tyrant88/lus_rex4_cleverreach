<?php
class rex_lus_cleverreach_utils {
	public static function getSettingsFile() {
		global $REX;

		$dataDir = $REX['INCLUDE_PATH'] . '/data/addons/lus_cleverreach/';
	...
	public static function updateSettingsFile($showSuccessMsg = true) {
		...
		if ($msg != '') {
			...
			foreach ((array) $REX['ADDON']['lus_cleverreach']['settings'] as $key => $value) {
				$content .= "\$REX['ADDON']['lus_cleverreach']['settings']['$key'] = " . var_export($value, true) . ";\n";
			}

			if (rex_put_file_contents($settingsFile, $content)) {
				if ($REX['REDAXO'] && $showSuccessMsg) {
					echo rex_info($I18N->msg('lus_cleverreach_config_ok'));
				}
			} else {
				if ($REX['REDAXO']) {
					echo rex_warning($I18N->msg('lus_cleverreach_config_error'));
				}
			}
		}
	}
	...
	public static function replaceSettings($settings) {
		...
		// type conversion
		foreach ($REX['ADDON']['lus_cleverreach']['settings'] as $key => $value) {
			if (isset($settings[$key])) {
				$settings[$key] = self::convertVarType($value, $settings[$key]);
			}
		}

		$REX['ADDON']['lus_cleverreach']['settings'] = array_merge((array) $REX['ADDON']['lus_cleverreach']['settings'], $settings);
	}
	...
	public static function checkDir($dir) {
		...
		if (!@is_dir($path)) {
			if ($REX['REDAXO']) {
				return $I18N->msg('lus_cleverreach_install_make_dir', $dir);
			}
		} elseif (!@is_writable($path . '/.')) {
			if ($REX['REDAXO']) {
				return $I18N->msg('lus_cleverreach_install_perm_dir', $dir);
			}
		}
